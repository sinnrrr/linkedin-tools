...
// let arr = Array.from(
//   document.querySelectorAll('button[id^="ember"].artdeco-button')
// );
...
// const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
// let arr = Array.from(document.querySelectorAll('button[aria-label^="Invite"]'));
// for (var i = 0; i < arr.length; i++) {
//   let btn = arr.pop();
//   let name = btn.getAttribute("aria-label").split(" ")[1];
//   const greeting = `Hello ${name}, I am expanding my network, it would be great if we could connect!
// By the way, ping me if you can refer me to your current company, that would be also super cool :)`;
...
Linkedin = {
  config: {
    ...
    addNote: true,
    note: `Hi {{name}}, I'm Dmytro, a 🇺🇦 full-stack developer, who recently moved to Vancouver 🇨🇦. I'm looking to connect with local professionals to expand my network. At the moment in an active job search, if you have any advice, job leads or contacts to share, I would greatly appreciate it. Thank you!`,
  },
